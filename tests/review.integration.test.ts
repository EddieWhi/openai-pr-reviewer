...
import {Options, PathFilter} from '../src/options'
import {ReviewContext} from '../src/review-context'
import {Prompts} from '../src/prompts'
import {OctokitPullRequest} from '../src/pull-request-impl'
...
test('test runs', async () => {
  const sharedOpenAiOptions = {
    ...
    timeoutMS: 12000,
    systemMessage:
      stripIndent(`You are \`@openai\` (aka \`github-actions[bot]\`), a language model 
  ...
  const pullRequest = await OctokitPullRequest.construct({
    owner: 'EddieWhi',
    repo: 'positional.rs.clone',
    pullNumber: 10
  })
