...
import {warning, logger} from './logger'
...
const octokit = new RetryAndThrottlingOctokit({
  ...
  throttle: {
    onRateLimit: (
      ...
      warning(
        `Request quota exhausted for request ${options.method} ${options.url}, Retry after: ${retryAfter} seconds, Retry count: ${retryCount}`
      )
      return true
    },
...
export interface OctokitPullRequestParams {
  owner: string
  repo: string
  pullNumber: number
}
...
export class OctokitPullRequest implements PullRequest {
  ...
  static async construct(
    params: OctokitPullRequestParams
  ): Promise<OctokitPullRequest> {
